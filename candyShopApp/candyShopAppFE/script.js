...
form.addEventListener("submit", async (e) => {
  try {
    ...
    //+10 button
    let plusTen = document.createElement("button");
    plusTen.id = `plusTen1`;
    plusTen.type = "submit";
    plusTen.className = "btn btn-outline-success btn-sm";
    plusTen.appendChild(document.createTextNode("+10"));

    //table data
    let td1 = document.createElement("td");
    td1.appendChild(plusTen);
      ...
      try {
        if (result.data.quantity > 0) {
          ...
          if (flag) {
            ...
            await axios.put(
              `http://localhost:3000/shop/${result.data.id}`,
              obj
            );
            location.reload();
          }
    ...
    plusTen.onclick = async () => {
      try {
        let finalQuantity = parseInt(result.data.quantity) + 10;
        const obj = {
          name,
          description,
          price,
